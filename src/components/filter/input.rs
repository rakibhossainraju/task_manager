...
pub fn Input() -> Element {
    ...
    rsx! {
        div { class: "flex items-center justify-start gap-2.5 px-4 bg-primary drop-shadow-default rounded border border-transparent focus-within:border-slate-300",
            span { dangerous_inner_html: SEARCH_ICON }
            div { class: "relative w-full text-slate-400 text-lg",
                input {
                    class: "w-full  py-4 text-lg outline-none border-none",
                    r#type: "text",
                    oninput: move |e| {
